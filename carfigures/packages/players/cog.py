    ...
    async def rebirth(self, interaction: discord.Interaction):
        ...
        if missing := set(
            ...
            await interaction.followup.send(
                ...
                f"Run the command `/{settings.cars_group_name} exhibit` to see how "
                f"many {settings.collectible_name}s you still need.",
                ephemeral=True,
            )
            return
        ...
        if player.rebirths >= 10:
            await interaction.followup.send(
                "You have already reached the maximum possible amount of rebirths."
            )
            return
        ...
        await interaction.followup.send(
            "Congratulations! To check the amount of rebirths you have done, run the command "
            f"`/{settings.player_group_name} profile`!"
        )
